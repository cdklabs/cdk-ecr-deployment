...
func TestGetRetryConfigs(t *testing.T) {
	testCases := []struct {
		...
		{
			name:                "successfully parses json with missing numAttempts",
		...
		{
			name:                "successfully parses json with missing baseDelay",
		...
		{
			name:                "successfully parses json with missing maxDelay",
...
func TestBackoffWithJitter(t *testing.T) {
	testCases := []struct {
		...
		{
			name:      "attempt 0 always returns [0, baseDelay]",
			attempt:   0,
			baseDelay: 1.0,
			maxDelay:  10.0,
			expectMin: time.Duration(0.0 * float64(time.Second)),
			expectMax: time.Duration(1.0 * float64(time.Second)),
		},
		{
			name:      "attempt 1 range is [0, baseDelay*2]",
			attempt:   1,
			baseDelay: 1.0,
			maxDelay:  10.0,
			expectMin: time.Duration(0.0 * float64(time.Second)),
		...
		{
			name:      "attempt 3 range is [0, baseDelay*8]",
			attempt:   3,
			baseDelay: 1.0,
			maxDelay:  30.0,
			expectMin: time.Duration(0.0 * float64(time.Second)),
			expectMax: time.Duration(8.0 * float64(time.Second)),
		},
		{
			name:      "delay is capped at [0, maxDelay]",
			attempt:   10,
			baseDelay: 1.0,
			maxDelay:  5.0,
			expectMin: time.Duration(0.0 * float64(time.Second)),
